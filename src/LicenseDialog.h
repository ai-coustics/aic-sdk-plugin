...
/// A simple error dialog component for displaying error messages.
///
...
{
  public:
    /// Callback function type for handling dialog close events.
    using CloseCallback = std::function<void()>;

    /// Constructs a new Error Dialog.
    ///
    /// # Arguments
    /// * `title` - The title text to display in the dialog
    ...
    /// Shows the error dialog as a modal component.
    ///
    /// # Arguments
    /// * `parentComponent` - The parent component to attach this dialog to
  ...
  private:
    ...
    /// Sets up the UI components with appropriate styling and properties.
    void setupComponents();

    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR(ErrorDialog)
};

/// A dialog component for entering and validating license keys.
///
...
{
  public:
    /// Callback function type for handling license key submission.
    ///
    /// # Arguments
    /// * `licenseKey` - The license key entered by the user
    ///
    /// # Returns
    /// `true` if the license key was accepted, `false` otherwise
    ...
    /// Constructs a new License Dialog.
    ///
    /// # Arguments
    /// * `callback` - Function to call when the user submits a license key
    /// * `isLicenseActive` - Whether a license is currently active (changes dialog text)
    ...
    /// Shows the dialog as a modal component.
    ///
    /// # Arguments
    /// * `parentComponent` - The parent component to attach this dialog to
    void showDialog(juce::Component* parentComponent);
    ...
    /// Sets focus to the license key text editor.
  ...
  private:
    ...
    /// Handles the OK button click or Enter key press.
    void handleOkAction();

    /// Handles the Cancel button click or Escape key press.
    void handleCancelAction();

    /// Sets up the UI components with appropriate styling and properties.
    void setupComponents();

    /// Shows a custom error dialog with the specified title and message.
    ///
    /// # Arguments
    /// * `title` - The title text to display in the error dialog
    /// * `message` - The error message to display
